...
public final class PolynomialTools {
    ...
    /**
     * This method counts a sum of two arrays element by element. If the arrays have different lengths,
     * then the smaller one is complemented with zeros, and only then their sum will be counted
     * @param coeffs1 first array 
     * @param coeffs2 second array
     * @return array, contained sums of each couple of elements on the same index
     */
    ...
    /**
     * This method works by appending zeros to a smaller of the given arrays, 
     * so that the lengths of the two given arrays will be the same. 
     * Important: arrays must be unequal.
     * How this works: the contents of the old array are copied to the beginning of the new one, 
     * and the missing number of elements stays as zeros.
    ...
    /**
     * This method multiplies two arrays with each other. 
     * Multiplication is performed according to the principle of polynomial multiplication. 
     * Each element of the first array is multiplied by each element of the second array, and then the coefficients 
     * at the same degrees are added and assigned to the new array in place of the resulting indexes 
     * (in this case a degree is an element index)  
     * @param coeffs1 fisrt array
     * @param coeffs2 second array
     * @return new array, which has a length as a sum of highest degrees (indexes) of two arrays
     */
